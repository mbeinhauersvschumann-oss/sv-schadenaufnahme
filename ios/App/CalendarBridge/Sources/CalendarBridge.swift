...
public class CalendarBridge: CAPPlugin {
    ...
    @objc func requestPermissions(_ call: CAPPluginCall) {
        store.requestAccess(to: .event) { granted, error in
            if let error = error { call.reject("perm error: \(error.localizedDescription)"); return }
    ...
    @objc func getEvents(_ call: CAPPluginCall) {
        guard let fromIso = call.getString("from"),
              let toIso   = call.getString("to") else { call.reject("missing from/to"); return }
        ...
        guard let start = f1.date(from: fromIso) ?? f2.date(from: fromIso),
              let end   = f1.date(from: toIso)   ?? f2.date(from: toIso) else { call.reject("bad date"); return }
    ...
    @objc func addEvent(_ call: CAPPluginCall) {
        guard let title = call.getString("title"),
              ...
              let endIso   = call.getString("end") else { call.reject("missing title/start/end"); return }

        let f = ISO8601DateFormatter()
        guard let s = f.date(from: startIso), let e = f.date(from: endIso) else { call.reject("bad date"); return }
        ...
        if let cid = call.getString("calendarId"), let cal = store.calendar(withIdentifier: cid) {
            ev.calendar = cal
        } else {
            ev.calendar = store.defaultCalendarForNewEvents
        }

        do { try store.save(ev, span: .thisEvent, commit: true); call.resolve(["id": ev.eventIdentifier ?? ""]) }
        catch { call.reject("save failed: \(error.localizedDescription)") }
    }

    @objc func updateEvent(_ call: CAPPluginCall) {
        guard let id = call.getString("id"),
              let ev = store.event(withIdentifier: id) else { call.reject("event not found"); return }
        ...
        if let cid = call.getString("calendarId"), let cal = store.calendar(withIdentifier: cid) { ev.calendar = cal }

        do { try store.save(ev, span: .thisEvent, commit: true); call.resolve(["ok": true]) }
    ...
    @objc func deleteEvent(_ call: CAPPluginCall) {
        guard let id = call.getString("id"),
              let ev = store.event(withIdentifier: id) else { call.reject("event not found"); return }
        do { try store.remove(ev, span: .thisEvent, commit: true); call.resolve(["ok": true]) }
        catch { call.reject("delete failed: \(error.localizedDescription)") }
    }
}
